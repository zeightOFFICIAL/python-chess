...
def redraw_gamewindow(board_to_render, player1_time, player2_time, state_white, state_black):
    ...
    if player1_time % 60 < 10:
        format_time_p1 = str(player1_time // 60) + ":" + \
            "0" + str(player1_time % 60)
    elif player1_time % 60 == 0:
        format_time_p1 += "0"
    if player2_time % 60 < 10:
        format_time_p2 = str(player2_time // 60) + ":" + \
            "0" + str(player2_time % 60)
...
def main():
    ...
    while run:
        ...
        wide_timer = time.time()
        redraw_gamewindow(game_board, int(player1_time), int(
            player2_time), statewhite, stateblack)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
...
# ----------------------------------------------------------------------------------------------------------------------
            if turn == "b" and game_mode == 1:
                ...
                try:
                    ...
                    print(
                        "log: type error, it's either critical script problem or true winning condition")
                    end_screen("White Wins!", time.time() - start_time)
                if change:
                    turn_number += 1
                    wide_timer = time.time()
